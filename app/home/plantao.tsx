...
import EditPlantaoModal from '@/components/EditPlantaoModal';
...
export default function PlantaoScreen() {
  ...
  const [records, setRecords] = useState<Plantao[]>([]);
  ...
  const loadRecords = useCallback(async () => {
    ...
    const data = await plantaoService.getByDate(dateStr);
    setRecords(data);
  }, [selectedDate]);
  ...
  const handleDelete = async (id: number) => {
    ...
    modal.show({
      ...
      onConfirm: async () => {
        try {
          await plantaoService.delete(id);
          await loadRecords();
        } catch (e: any) {
  ...
  const handleEdit = (item: Plantao) => {
    closeSwipeable();
    setEditingPlantao(item);
    setEditModalVisible(true);
  };

  const handleEditSave = async (id: number | null, startTime: string, endTime: string, description: string | null) => {
    try {
      const date = formatDateToISO(selectedDate);
      if (id !== null) {
        await plantaoService.update(id, date, startTime, endTime, description);
      } else {
        await plantaoService.create(date, startTime, endTime, description);
      }
  ...
  const renderRightActions = (item: Plantao) => {
    return (
  ...
  const renderItem = ({ item, index }: { item: Plantao; index: number }) => {
      ...
      >
        ...
        >
          <View style={[styles.badge, { backgroundColor: accentColor + '18' }]}>
            <FontAwesome5 name="briefcase-medical" size={16} color={accentColor} />
          </View>
          <View style={styles.cardContent}>
            ...
            {item.description ? (
              <Text style={[styles.cardDescription, isDark && styles.textDark]} numberOfLines={1}>
                {item.description}
              </Text>
            ) : null}
          </View>
          <Text style={[styles.cardTime, isDark && styles.textDark]}>
            {item.total_time.slice(0, 5)}
          </Text>
        </View>
      </Swipeable>
    );
  ...
  const renderEmpty = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={[styles.emptySubtext, isDark && styles.emptySubtextDark]}>
        Toque no botão para registrar um plantão
      </Text>
    </View>
  );
      ...
      <View style={[styles.container, isDark ? styles.containerDark : styles.containerLight]}>
        <View style={styles.dateBar}>
          ...
          <Pressable onPress={() => setShowPicker(true)} style={styles.dateBadge}>
            <FontAwesome5 name="calendar-alt" size={14} color="#f59e0b" style={{ marginRight: 8 }} />
        ...
        <Pressable
          onPress={() => { setEditingPlantao(null); setEditModalVisible(true); }}
          style={({ pressed }) => [
            styles.fab,
        ...
        <EditPlantaoModal
          ...
          onClose={() => { setEditModalVisible(false); setEditingPlantao(null); }}
        />
        <CustomToast
